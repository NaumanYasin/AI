...
def function8():
    ...
    x = np.multiply(np.ones(10, dtype = np.uint32).reshape(2,5),6) #write your code here
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = arr - brr[0:,None] # write your code here 
...
def function11():
    ...
    ans = np.copy(arr); ans[ans % 2 == 1] = - 1  #write your code here 
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.stack((arr,arr,arr), axis = -1); ans = np.concatenate((ans.flatten(),arr,arr,arr), axis = 0) #write your code here 
...
def function16():
    ...
    x = np.array([[1], [2], [3]])
    y = np.array([[2], [3], [4]])
    ans = [np.concatenate((x.tolist(),y.tolist()), axis = -1).tolist()] #write your code here 
